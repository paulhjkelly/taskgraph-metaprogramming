...
/***************************************************************************
 *  return sign of fm_constraint.                                          *
 ***************************************************************************/
int sc_fm_constraint::sign(int in)
{
    // jump out of solver if sign is bad
    if ((sgn[in] == BAD_SIGN) && _named_sc_fm_longjmp_env)
        longjmp(*_named_sc_fm_longjmp_env, 1);

    assert(sgn[in]!=BAD_SIGN);
    return sgn[in];
}
 ...
 ***************************************************************************/
void sc_fm_constraint::print(FILE *fp)
{
    fprintf(fp,"rank=%d, plane_rank=%d,  magic=%d\n", 
           rank(), 
           plane_rank(), 
           magic_num);
    for(int in=0; in<n(); in++) {
        if(sgn[in] == BAD_SIGN) 
            ...
            fprintf(fp,"%2d: ", sgn[in]);
        for(int ip=0; ip<p(); ip++)
            fprintf(fp,"%2d ", (*this)[ip][in]);
        fprintf(fp,"\n");
    }
}
...
boolean named_sc_fm::fm_step(int i, int j)
{
    ...
    if(MAX(1, i)<j)
        for(int ii=0; ii<m(); ii++)
            if(check_valid(ii) == FALSE) return FALSE;
...
boolean named_sc_fm::fm_step(int j)
{
    ...
    for(int ix=0; ix<m(); ix++)
        fprintf(stderr,"%d ", (*this)[ix].sign(j));
    fprintf(stderr," ]\n");
#endif
...
#ifdef DEBUG_PRINT
    fprintf(stderr,"fm_step---%d(%s)--- [oldM=%d, #chk=%d -> newM=%d]\n", 
           j, get_name(nm_c[j]),
        ...
        if((*this)[l].sign(j) > 0) 
            for(int u=ua; u<ub; u++)
                if((*this)[u].sign(j) < 0) {
                    int tm = xm++;
#ifdef DEBUG_PRINT3
                    fprintf(stderr," (%d %d)%d", l, u, tm); fflush(stderr);
#endif
...
#ifdef DEBUG_PRINT3
                    fprintf(stderr,"%s", (*this)[tm].is_only_pos_const()?"C":"");
                    if(is_already_covered(tm)) fprintf(stderr,"I(%d)", tmp_var);
#endif
                           ...
                           sc_fm_constraint & B)
{
    ...
    if(cxa.rank() == 0) {
        ...
        if(B.plane_rank() != 0) {
            if(_named_sc_fm_longjmp_env)
                longjmp(*_named_sc_fm_longjmp_env, 1);
            fprintf(stderr,"Cannot reduce, too complicated (1)\n");
            fprintf(stderr,"j=%d\n", j);
            fprintf(stderr,"A\n");   A.print(stderr);
            fprintf(stderr,"B\n");   B.print(stderr);
            fprintf(stderr,"as=%d bs=%d\nca=%d cb=%d\n", as, bs, ca, cb);
            fprintf(stderr,"cxa = "); cxa.print(stderr);
            fprintf(stderr,"cxb = "); cxb.print(stderr);
            fprintf(stderr,"cc  = "); cc.print(stderr); 
            error();
        }
    ...
    if(cxb.rank() == 0) {
        ...
        if(A.plane_rank() != 0) {
            if(_named_sc_fm_longjmp_env) 
                longjmp(*_named_sc_fm_longjmp_env, 1);
            fprintf(stderr,"Cannot reduce, too complicated (2)\n");
            fprintf(stderr,"j=%d\n", j);
            fprintf(stderr,"A\n");   A.print(stderr);
            fprintf(stderr,"B\n");   B.print(stderr);
            fprintf(stderr,"as=%d bs=%d\nca=%d cb=%d\n", as, bs, ca, cb);
...
boolean named_sc_fm::fm_lin_step()
{
    ...
    fprintf(stderr,"fm_step---lin-- ineqs=%d -\n", lla.m());
    leq.print_exp(pet_system_nl,stderr);
#endif
...
#ifdef DEBUG_PRINT
    fprintf(stderr,"@@@@@@@@@@@@@F@M@@P@R@O@J@E@C@T@@@@@@@@@@@@@@@@\n");
    sort();
    print(stderr);
#endif
    assert((0<i)&&(i<=n()));
...
#ifdef DEBUG_PRINT
    fprintf(stderr,"@@@@@@@@@@@@@@@@@@after fm_project(%d, %d)@@@@@@@@@@@@@@@@@@@@@\n", i, j);
    sort();
    print(stderr);
#endif
...
#ifdef DEBUG_PRINT
    fprintf(stderr,"@@@@@@@@@@@@@F@M@@B@O@U@N@D@S@@@@@@@@@@@@@@@@@@\n");
    sort();
    print(stderr);
#endif
    assert((0<i)&&(i<=n()));
...
#ifdef DEBUG_PRINT
    fprintf(stderr,"@@@@@@@@@@@@@@@@@@after fm_step(%d, %d)@@@@@@@@@@@@@@@@@@@@@\n", i, j);
    sort();
    print(stderr);
#endif
...
void named_sc_fm::fm_bounds_do(int j, boolean no_del)
{
    ...
    fprintf(stderr,"   %d--rnk--e-l-u-del--- numeq=%d simpeq=\n", 
           j, numeq, simpeq);
    for(i=0; i<tmp->m(); i++) {
        fprintf(stderr,"%2d    %d    %d %s %s  %s   ", 
               i, 
               dumb[i], 
               s_eq[i],
               (s_l[i])?"*":" ", 
        ...
        tmp->print_exp(i,stderr); fprintf(stderr,"  >= 0\n");
    }
    delete tmp;
    delete tmpnew;
#endif
 ...
 ***************************************************************************/
void named_sc_fm::print(FILE *fp)
{
    fprintf(fp,"Internal:\n");
    named_symcoeff_ineq * tmpi = internal_get();
    tmpi->print_exp(pet_system_nl,fp);
    fprintf(fp,"Results:\n");
    named_symcoeff_ineq * tmpr = get();
    tmpr->print_exp(pet_system_nl,fp);
    delete tmpi;
    delete tmpr;
}
