 ...
 *                                                                              *
 ********************************************************************************/
int lin_ineq_op::print_code(const lin_ineq & a, const lin_ineq * stride,
                            name_store * nm, FILE * fp) const
{
    return new_print_code(a, stride, nm->params, fp);
}




void lin_ineq_op::single_update(lin_ineq & a, constraint repl, int rnk) const
{
/*    lin_ineq rlow(a);
    for(int i = 0; i <a.m(); i++)
...
static void print_constraint(char * buf, char ** name,
                             const constraint & curr, int /* divnum */ [])
{
    // BUGBUG: Parameter divnum should be used here.
    char tmpbuf[32];
    int dosign = 0;
    if(curr.c(0) != 0) {
        sprintf(tmpbuf, "%d", curr.c(0));
        strcat(buf, tmpbuf);
        dosign = 1;
    }
...
void lin_ineq_op::print_code(lin_ineq a, const lin_ineq * strides,
                             char ** name, FILE * fp) const
{
    char buf[255];
    char tmpbuf[32];
    int  divnum[32];
    ...
    for(i=0; i < cnt; i++) {
		...
		int j;
                ...
                if(strides) {
                    ...
                    st_filt[myrank] = 1;
                    lin_ineq curr_stride = strides->filter_thru(st_filt, -1);
                    if(curr_stride.m() == 0)
		    ...
		    int j;
                    for(j=0; j < lo.m(); j++) {             // All of constraints
                        if(lo.m() > 1) {
                            if(j != 0) strcat(buf, ", ");
                            if(j != lo.m()-1)
...
public:
    ...
    code_tree_node(const char * v) { init(); op = NO_VAR; var = v; }
...
public:
    ...
    code_nest * make_sib();
    int print(FILE * fp=stderr, int tab=0);

    code_tree_node * create_code_tree(const constraint & val,
                                      char ** name, int lev,
                                      node_op o);
    code_tree_node * create_code_tree(const lin_ineq & val,
                                      char ** name, int lev,
                                      node_op o);
};
...
code_tree_node * code_nest::create_code_tree(const constraint & val,
                                             char ** name,
                                             int lev,
                                             node_op cf)
{
    int sign = (val.c(lev) > 0)?-1:1;
    code_tree_node * Curr = new code_tree_node(val.c(0)*sign);

    for(int i=1; i<lev; i++)
        if(val.c(i) != 0) {
            code_tree_node * coif  = new code_tree_node(val.c(i)*sign);
    ...
    if(ABS(val.c(lev)) != 1) {
        code_tree_node * coif  = new code_tree_node(-1*sign*val.c(lev));
...
code_tree_node * code_nest::create_code_tree(const lin_ineq & val,
                                             char ** name,
                                             int lev,
                                             node_op mm)
{
    ...
    for(int i=0; i<val.m(); i++) {
        code_tree_node * a = create_code_tree(val.r(i), name, lev, cf);
        if(Curr == 0)
            Curr = a;
        else 
...
int lin_ineq_op::new_print_code(const lin_ineq & a,
                                const lin_ineq * strides,
                                char ** name,
                                FILE * fp) const
{
...
void lin_ineq::sprint_eq(char * buf, const char * names[], int upper, int lhs)
        const
{
    assert(buf);
    strcpy(buf, "");
    ...
    for(i=0; i<m(); i++) {
        ...
        if(lhs > 0)
            if(ABS(r(i).c(lhs)) != 1) {
                sprintf(tmpbuf, ", %d)", ABS(r(i).c(lhs)));
                strcat(buf, tmpbuf);
            }
