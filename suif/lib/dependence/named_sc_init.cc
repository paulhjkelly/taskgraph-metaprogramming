...
named_symcoeff_ineq::named_symcoeff_ineq(named_symcoeff_ineq & c)
{
    init();
    init(c);
}
...
named_symcoeff_ineq::named_symcoeff_ineq(named_lin_ineq & cc)
{
    init();
    init(cc);
}
 ...
 ***************************************************************************/
void named_symcoeff_ineq::initL(named_lin_ineq * l0, named_lin_ineq * l1, 
 ...
 ***************************************************************************/
void named_symcoeff_ineq::init(named_symcoeff_ineq & c)
{
    nt_c.init(c.cols());
    nt_p.init(c.planes());
 ...
 ***************************************************************************/
void named_symcoeff_ineq::init(named_lin_ineq * l0,
                              immed * v1, named_lin_ineq * l1,
                              immed * v2, named_lin_ineq * l2,
                              immed * v3, named_lin_ineq * l3,
                              immed * v4, named_lin_ineq * l4,
 ...
 ***************************************************************************/
void named_symcoeff_ineq::init(named_lin_ineq & cc)
{
    init(&cc);
}
 ...
 ***************************************************************************/
int named_symcoeff_ineq::init(immed_list & il, int c=0)
{
    unum = il[c++].integer();
    ...
    cols().init(num);
    for(i=1; i<num; i++)
       cols()[i].init((name_table_entry_kind)il[c++].integer(),il[c++]);

    num = il[c++].integer();
    planes().init(num);
    for(i=1; i<num; i++)
       planes()[i].init((name_table_entry_kind)il[c++].integer(),il[c++]);
    L = new lin_ineq [num];
    for(i=0; i<num; i++) 
...
immed_list * named_symcoeff_ineq::cvt_immed_list()
{
    ...
    lstL->append(immed(n()));
    for(i=1; i<n(); i++) {
        lstL->append(immed(cols()[i].kind()));
        lstL->append(cols()[i].name());
    }
    lstL->append(immed(p()));
    for(i=1; i<p(); i++) {
        lstL->append(immed(planes()[i].kind()));
 ...
 ***************************************************************************/
lin_ineq * named_symcoeff_ineq::get_p(int i)
{
    lin_ineq * ret = new lin_ineq((*this)[i]);
    return ret;
}


lin_ineq * named_symcoeff_ineq::get_m(int i)
{
    lin_ineq * ret = new lin_ineq(p(), n());
    for(int ip=0; ip<p(); ip++)
        for(int in=0; in<n(); in++)
            (*ret)[ip][in] = (*this)[ip][i][in];
    return ret;
}


lin_ineq * named_symcoeff_ineq::get_n(int i)
{
    lin_ineq * ret = new lin_ineq(p(), m());
    for(int ip=0; ip<p(); ip++)
        for(int im=0; im<m(); im++)
            (*ret)[ip][im] = (*this)[ip][im][i];
    return ret;
}
...
constraint * named_symcoeff_ineq::get_pm(int ip, int im)
{
    constraint * ret = new constraint(n());
    for(int in=0; in<n(); in++)
        (*ret)[in] = (*this)[ip][im][in];
    return ret;
}

constraint * named_symcoeff_ineq::get_pn(int ip, int in)
{
    constraint * ret = new constraint(m());
    for(int im=0; im<m(); im++)
        (*ret)[im] = (*this)[ip][im][in];
    return ret;
}


constraint * named_symcoeff_ineq::get_mn(int im, int in)
{
    constraint * ret = new constraint(p());
    for(int ip=0; ip<p(); ip++)
        (*ret)[ip] = (*this)[ip][im][in];
    return ret;
}
